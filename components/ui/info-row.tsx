...
export function InfoRow({ label, value, href, note }: InfoRowProps) {
  ...
  const content = href ? (
    <a
      className="group inline-flex items-center gap-2 rounded-full border border-brand-stone/90 bg-brand-shell/88 px-4 py-2 text-brand-ink transition-all duration-300 ease-out hover:-translate-y-0.5 hover:border-brand-red/45 hover:bg-brand-rose hover:text-brand-noir hover:shadow-soft"
      href={href}
